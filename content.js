...
async function fillPrevTravel(data) {
  ...
  // ¿Cancelación de visa? — usa mismo campo PUST_EXP_RECHAZO
  if (data.visaRefusedExplanation && !isBlank(data.visaRefusedExplanation)) {
    fillRadio(id('rblPREV_VISA_CANCELLED_IND_0'));
  } else {
...
if (data.socialNetwork && data.socialNetwork !== 'NONE') {
    ...
    // Esperar a que el campo se habilite tras seleccionar la red social
    await new Promise(r => setTimeout(r, 500));
    const socialField = document.getElementById(id('dtlSocial_ctl00_tbxSocialMediaIdent'));
    if (socialField && !socialField.disabled) {
...
async function fillPassport(data) {
  ...
  if (data.passportLostNumber && !isBlank(data.passportLostNumber) &&
      data.passportLostExplanation && !isBlank(data.passportLostExplanation)) {
...
async function fillContact(data) {
  ...
  if (data.usContactHotel && !isBlank(data.usContactHotel)) {
  ...
  if (fillInput( id('tbxUS_POC_ADDR_POSTAL_CD'),   data.usContactZip))    ok++;
...
console.log('[VP] hasOtherUSRelative raw:', JSON.stringify(data.hasOtherUSRelative));
  const hasOther = (data.hasOtherUSRelative || '').trim().toUpperCase()
    .normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  console.log('[VP] hasOther procesado:', JSON.stringify(hasOther));
...
async function fillWork(data) {
  ...
  // Si no existen los campos de trabajo, hacer postback via fetch
  if (!document.getElementById(id('tbxEmpSchName'))) {
    const form = document.forms[0];
    const formData = new FormData(form);
    formData.set('__EVENTTARGET', 'ctl00$SiteContentPlaceHolder$FormView1$ddlPresentOccupation');
    formData.set('__EVENTARGUMENT', '');
    formData.set('ctl00$SiteContentPlaceHolder$FormView1$ddlPresentOccupation', 'O');
    ...
    // Esperar a que el campo esté disponible tras actualizar el DOM
    try { await waitFor(id('tbxEmpSchName')); } catch(e) {}
  }

  // Llenar todos los campos
  if (fillInput(id('tbxExplainOtherPresentOccupation'), 
      data.occupation === 'O' ? 'EMPLOYEE' :       data.occupation)) ok++;
...
const SECURITY_PARTS = [
  ...
  // Part 4
  ['rblImmigrationFraud_1','rblDeport_1'],
  // Part 5
  ['rblChildCustody_1','rblVotingViolation_1','rblRenounceExp_1'],
];
